...
class Ingredient(BaseModel):
    """
    Model representing a single ingredient with amount, unit, and name.
    """
    amount: float = Field(description="Amount of the ingredient")
    unit: str = Field(description="Unit of measurement (e.g., cups, tablespoons)")
...
def get_recipe_from_text(recipe_text: str) -> Recipe:
    ...
    response = client.responses.parse(
        ...
        input=[
            {"role": "user", "content": f"You can only talk English to me. Convert this recipe into the specified format:\n\n{recipe_text}"}
        ],
        text_format=Recipe
    )
...
if __name__ == "__main__":
    ...
    recipe_path = os.path.join(script_dir, "waterzooi.txt")
